...
COMMENT ON COLUMN "Nova".t0116.discount_type IS 'Discount application type: FixedPrice | Percentage | FixedDiscount';
...
COMMENT ON COLUMN "Nova".t0117.customer_group IS 'Customer group designation (e.g. Wholesale, HoReCa Tier A, Supermarket, Retail)';
COMMENT ON COLUMN "Nova".t0117.price_list_id IS 'Assigned default price list (FK to t0083)';
COMMENT ON COLUMN "Nova".t0117.priority IS 'Priority ranking when resolving overlapping rules (lower/higher priority evaluation)';
...
ALTER TABLE "Nova".t0013
    ...
    ADD COLUMN IF NOT EXISTS applied_promotion_id INT REFERENCES "Nova".t0119(id) ON DELETE SET NULL,
