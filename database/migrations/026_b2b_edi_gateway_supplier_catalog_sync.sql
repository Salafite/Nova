...
-- Migration 026: EDI Trading Partners (T0134), EDI SKU Cross-Reference Matrix (T0135),
--                EDI Transaction / Interchange Logs (T0136), EDI SSCC Pallet Logistics (T0137),
...
-- 2. EDI Trading Partners Table (T0134)
...
COMMENT ON COLUMN "Nova".t0134.segment_terminator IS 'Segment delimiter character (e.g. ~ or CHR(39) or \n)';
...
-- 3. EDI SKU Cross-Reference Matrix Table (T0135)
...
-- 4. EDI Transaction / Interchange Logs Table (T0136)
...
-- 5. EDI SSCC Pallet Logistics Table (T0137)
...
-- 6. Supplier Catalog Sync / 832 PRICAT Table (T0138)
...
DO $$ BEGIN
    IF EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'nova_readonly') THEN
        GRANT SELECT ON "Nova".t0134 TO nova_readonly;
        GRANT SELECT ON "Nova".t0135 TO nova_readonly;
        GRANT SELECT ON "Nova".t0136 TO nova_readonly;
