...
-- Migration 025: Volume Tier Breaks (T0120), Customer Group Price Lists (T0121),
--                Customer Contracts (T0122), Promotional Campaign Rules (T0123)
BEGIN;
...
COMMENT ON SEQUENCE "Nova".seq_promo_code IS 'Atomic sequence for generating unique promotional campaign codes (PROMO-XXXXX)';

-- 2. Volume Tier Breaks Table (T0120)
...
-- 3. Customer Group Price Lists Matrix Table (T0121)
...
COMMENT ON COLUMN "Nova".t0121.customer_group IS 'Customer group designation (e.g. Wholesale, HoReCa, Supermarket, Retail)';
COMMENT ON COLUMN "Nova".t0121.price_list_id IS 'Assigned default price list (FK to t0083)';
COMMENT ON COLUMN "Nova".t0121.priority IS 'Priority ranking when resolving overlapping rules (lower = higher priority)';
...
-- 4. Customer Contracts Table (T0122)
...
COMMENT ON TABLE "Nova".t0122 IS 'Customer Contracts — Special contracted price overrides and commitment terms';
...
-- 5. Promotional Campaign Rules Table (T0123)
...
COMMENT ON TABLE "Nova".t0123 IS 'Promotional Campaign Rules — Time-bounded promotions and Buy-X-Get-Y campaign rules';
COMMENT ON COLUMN "Nova".t0123.code IS 'Unique promotion code identifier (PROMO-XXXXX)';
COMMENT ON COLUMN "Nova".t0123.promo_type IS 'Promotion mechanism: BUY_X_GET_Y | PERCENTAGE_DISCOUNT | FLAT_DISCOUNT';
COMMENT ON COLUMN "Nova".t0123.buy_product_id IS 'Required trigger product reference (FK to t0001)';
COMMENT ON COLUMN "Nova".t0123.buy_min_qty IS 'Minimum purchase quantity required to trigger promo';
COMMENT ON COLUMN "Nova".t0123.get_product_id IS 'Reward product item reference (FK to t0001)';
COMMENT ON COLUMN "Nova".t0123.get_qty IS 'Quantity of reward items provided per trigger unit';
COMMENT ON COLUMN "Nova".t0123.discount_percent IS 'Discount percentage applied to reward item (100.00 = Free)';
COMMENT ON COLUMN "Nova".t0123.discount_amount IS 'Fixed monetary discount applied if percentage discount not used';
...
COMMENT ON COLUMN "Nova".t0123.price_list_id IS 'Target price list filter (NULL = applies to all price lists)';
