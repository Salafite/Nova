...
-- Migration 025: Tier Breaks (T0084), Customer Group Price Lists (T0120), Promotional Campaign Rules (T0121)
BEGIN;
...
COMMENT ON TABLE "Nova".t0120 IS 'Customer Group Price List Mappings — Default price list matrix per customer segment (e.g., HoReCa Tier A, Wholesale, Supermarket)';
COMMENT ON COLUMN "Nova".t0120.id IS 'Primary key';
COMMENT ON COLUMN "Nova".t0120.customer_group IS 'Customer group / segment name (e.g. HoReCa Tier A, Wholesale, Supermarket)';
COMMENT ON COLUMN "Nova".t0120.price_list_id IS 'Assigned default price list (FK to t0083)';
COMMENT ON COLUMN "Nova".t0120.priority IS 'Resolution priority when multiple group rules match (lower number = higher priority)';
COMMENT ON COLUMN "Nova".t0120.business_id IS 'Tenant / business organization identifier (FK to T0059)';

CREATE INDEX IF NOT EXISTS idx_t0120_customer_group ON "Nova".t0120(customer_group);
...
COMMENT ON TABLE "Nova".t0121 IS 'Promotional Campaign Rules — Time-bounded promotions and Buy-X-Get-Y campaign rules';
COMMENT ON COLUMN "Nova".t0121.id IS 'Primary key';
COMMENT ON COLUMN "Nova".t0121.code IS 'Unique promotion campaign code (PROMO-XXXXX)';
COMMENT ON COLUMN "Nova".t0121.name IS 'Promotion rule name / campaign title';
COMMENT ON COLUMN "Nova".t0121.promo_type IS 'Promotion rule mechanism: BUY_X_GET_Y | PERCENTAGE_DISCOUNT | FLAT_DISCOUNT';
COMMENT ON COLUMN "Nova".t0121.buy_product_id IS 'Required trigger product reference (FK to T0003)';
COMMENT ON COLUMN "Nova".t0121.buy_min_qty IS 'Minimum purchase quantity required to trigger promo';
COMMENT ON COLUMN "Nova".t0121.get_product_id IS 'Bonus / reward item product reference (FK to T0003)';
COMMENT ON COLUMN "Nova".t0121.get_qty IS 'Quantity of reward items provided per trigger unit';
COMMENT ON COLUMN "Nova".t0121.discount_percent IS 'Discount percentage applied to get item (100.00 = 100% Free)';
COMMENT ON COLUMN "Nova".t0121.discount_amount IS 'Fixed monetary discount applied if percentage discount not used';
COMMENT ON COLUMN "Nova".t0121.customer_group IS 'Target customer group filter (NULL = applies to all groups)';
COMMENT ON COLUMN "Nova".t0121.price_list_id IS 'Target price list filter (NULL = applies to all price lists)';
COMMENT ON COLUMN "Nova".t0121.start_date IS 'Promotional campaign validity period start date & time';
COMMENT ON COLUMN "Nova".t0121.end_date IS 'Promotional campaign validity period end date & time';
COMMENT ON COLUMN "Nova".t0121.business_id IS 'Tenant / business organization identifier (FK to T0059)';

CREATE INDEX IF NOT EXISTS idx_t0121_code ON "Nova".t0121(code);
CREATE INDEX IF NOT EXISTS idx_t0121_buy_product_id ON "Nova".t0121(buy_product_id);
CREATE INDEX IF NOT EXISTS idx_t0121_get_product_id ON "Nova".t0121(get_product_id);
CREATE INDEX IF NOT EXISTS idx_t0121_validity ON "Nova".t0121(start_date, end_date);
CREATE INDEX IF NOT EXISTS idx_t0121_active ON "Nova".t0121(is_active);
...
ALTER TABLE "Nova".t0013
    ...
    ADD COLUMN IF NOT EXISTS applied_promotion_id INT REFERENCES "Nova".t0121(id) ON DELETE SET NULL,
