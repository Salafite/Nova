window.JournalEntryService = class JournalEntryService {
  constructor() {
    this.api = new ApiClient('/api/T0027I')
    this._data = []
  }

  async load() {
    try { this._data = await this.api.list() || [] } catch (e) { console.warn('Failed to load journal entries', e); this._data = [] }
  }
  ...
  async postJE(id) { return this.update(id, { status: 'Posted' }) }

  getPendingCount() { return this._data.filter(function(p) { return p.status === 'Draft' }).length }
}
