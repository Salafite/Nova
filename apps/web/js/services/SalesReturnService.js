window.SalesReturnService = class SalesReturnService {
  constructor() {
    this.api = new ApiClient('/api/T0079I');
    this._data = [];
  }

  async load() {
    try { this._data = await this.api.list() || [] } catch (e) { console.warn('Failed to load sales returns', e); this._data = [] }
  }
  ...
  getPendingCount() { return this._data.filter(function(r) { return r.status !== 'Received' && r.status !== 'Cancelled' }).length }
}
