window.ProjectService = class ProjectService {
  constructor() {
    this.api = new ApiClient('T0044I')
    this._data = []
  }
  async load() {
    try {
  ...
  getActiveCount() {
    return this._data.filter(function(x) { return x.status === 'In Progress' || x.status === 'Active' }).length
  }
  getCompletionRate() {
    var total = this._data.length
    if (!total) return 0
    var completed = this._data.filter(function(x) { return x.status === 'Completed' }).length
