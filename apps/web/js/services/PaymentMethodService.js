window.PaymentMethodService = class PaymentMethodService {
  constructor() {
    this.api = new ApiClient('T0097I')
    this._data = []
  }

  async load() {
    try { this._data = await this.api.list() || [] } catch (e) { console.warn('Failed to load payment methods', e); this._data = [] }
  }
  ...
  toggleDefault(id) {
    var item = this.getById(id)
    if (!item) return
    return this.update(id, { isDefault: !item.isDefault })
  }
}
