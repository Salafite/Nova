window.ComplianceService = class ComplianceService {
  constructor() {
    this.api = new ApiClient('T0063I')
    this._data = []
  }
  async load() {
    try {
      ...
      console.warn('Failed to load compliance rules', e)
      this._data = []
    }
  }
  ...
  getHighRiskCount() {
    return this._data.filter(function(x) { return x.riskLevel === 'High' || x.riskLevel === 'Critical' }).length
  }
}
