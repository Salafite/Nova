window.TaxRateService = class TaxRateService {
  constructor() {
    this.api = new ApiClient('/api/T0085I');
    this._data = [];
  }

  async load() {
    try { this._data = await this.api.list() || [] } catch (e) { console.warn('Failed to load tax rates', e); this._data = [] }
  }
  ...
  getActiveCount() { return this._data.filter(function(r) { return r.isActive }).length }

  getDefaultCount() { return this._data.filter(function(r) { return r.isDefault }).length }
}
