window.GoodsReceiptService = class GoodsReceiptService {
  constructor() {
    this.api = new ApiClient('/api/T0075I');
    this._data = [];
  }

  async load() {
    try { this._data = await this.api.list() || [] } catch (e) { console.warn('Failed to load goods receipts', e); this._data = [] }
  }
  ...
  getPendingCount() { return this._data.filter(function(r) { return r.status !== 'Completed' && r.status !== 'Cancelled' }).length }
}
