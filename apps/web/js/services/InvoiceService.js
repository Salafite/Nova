window.InvoiceService = class InvoiceService {
  constructor() {
    this.api = new ApiClient('T0090I')
    this._data = []
  }

  async load() {
    try { this._data = await this.api.list() || [] } catch (e) { console.warn('Failed to load invoices', e); this._data = [] }
  }
  ...
  getPendingCount() { return this._data.filter(function(p) { return p.status === 'Unpaid' }).length }
}
