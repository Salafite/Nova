window.RFQService = class RFQService {
  constructor() {
    this.api = new ApiClient('/api/T0071I');
    this._data = [];
  }

  async load() {
    try { this._data = await this.api.list() || [] } catch (e) { console.warn('Failed to load RFQs', e); this._data = [] }
  }
  ...
  async convertToPO(id, vendorId) {
    return await this.api._fetch(this.api.base + '/' + id + '/convert?vendor_id=' + vendorId, { method: 'POST' })
  }
}
