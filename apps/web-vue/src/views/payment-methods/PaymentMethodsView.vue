<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div>
        <h1 class="page-title">{{ t('paymethod-title') }}</h1>
        <p class="page-subtitle">{{ t('paymethod-sub') }}</p>
      </div>
      <button class="btn-primary" @click="openAdd">
        <span class="material-symbols-outlined">add</span> {{ t('new-paymethod') }}
      </button>
    </div>
        ...
        <table class="data-table">
          <thead>
            <tr>
              ...
              <th>{{ t('name') }}</th>
              <th>{{ t('description') }}</th>
              <th class="text-center">{{ t('paymethod-default') }}</th>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content">
        <div class="modal-header">
          <h3>{{ editing ? t('edit-paymethod') : t('new-paymethod') }}</h3>
    ...
    <div v-if="showDelete" class="modal-overlay" @click.self="showDelete = false">
      <div class="modal-content modal-sm">
        ...
        <div class="modal-body">
          <p class="delete-text">{{ t('paymethod-delete-confirm') }} <strong>{{ deleteTarget?.name }}</strong>?</p>
          <div class="modal-actions">
...
const form = ref({ name: '', code: '', description: '', is_active: true, is_default: false })
...
async function load() {
  ...
  error.value = ''
  try {
    const res = await api.get('/T0097I/')
...
function openAdd() {
  editing.value = false
  editId.value = null
  form.value = { name: '', code: '', description: '', is_active: true, is_default: false }
...
async function saveItem() {
  ...
  try {
    ...
    if (editing.value) {
      await api.put(`/T0097I/${editId.value}`, payload)
      toast('Payment Method ' + t('saved-ok'), 'success')
    } else {
      await api.post('/T0097I/', payload)
      toast('Payment Method ' + t('saved-ok'), 'success')
    }
    closeModal()
    await load()
  } catch {
    toast(t('failed-save') + ' Payment Method', 'error')
  } finally {
    saving.value = false
  }
}
...
async function confirmDelete() {
  deleting.value = true
  try {
    await api.delete(`/T0097I/${deleteTarget.value.id}`)
...
.cell-desc { color: #666; font-size: 12px; max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
