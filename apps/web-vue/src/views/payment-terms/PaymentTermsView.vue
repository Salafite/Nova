<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div>
        <h2 class="page-title">{{ t('payterm-title') }}</h2>
        <p class="page-subtitle">{{ t('payterm-sub') }}</p>
      </div>
      <button class="btn-primary" @click="openAdd">
        <span class="material-symbols-outlined">add</span> {{ t('new-payterm') }}
      </button>
    </div>
        ...
        <table class="data-table">
          <thead>
            <tr>
              <th>{{ t('code') }}</th>
              <th>{{ t('name') }}</th>
              <th>{{ t('payterm-due-days') }}</th>
              <th>{{ t('payterm-discount') }}</th>
              <th>{{ t('payterm-discount-days') }}</th>
              <th class="text-center">{{ t('payterm-default') }}</th>
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              ...
              <td>{{ item.due_days }}d</td>
              <td>{{ item.discount_percentage }}%</td>
              <td>{{ item.discount_days }}d</td>
              <td class="text-center">
                <span v-if="item.is_default" class="material-symbols-outlined text-green-600">check_circle</span>
                <span v-else class="material-symbols-outlined text-gray-300">radio_button_unchecked</span>
              </td>
              <td class="text-center">
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content">
        <div class="modal-header">
          <h3>{{ editing ? t('edit-payterm') : t('new-payterm') }}</h3>
        ...
        <div class="modal-body">
          <div class="form-row">
            <div class="form-group">
              ...
              <input type="text" v-model="form.code" class="form-input" maxlength="20" />
            </div>
            <div class="form-group">
          ...
          <div class="form-row">
            <div class="form-group">
              <label>{{ t('payterm-due-days') }}</label>
              <input type="number" min="0" v-model.number="form.due_days" class="form-input" />
            </div>
            <div class="form-group">
              <label>{{ t('payterm-discount') }} (%)</label>
              <input type="number" step="0.01" min="0" max="100" v-model.number="form.discount_percentage" class="form-input" />
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label>{{ t('payterm-discount-days') }}</label>
              <input type="number" min="0" v-model.number="form.discount_days" class="form-input" />
            </div>
          </div>
    ...
    <div v-if="showDelete" class="modal-overlay" @click.self="showDelete = false">
      <div class="modal-content modal-sm">
        ...
        <div class="modal-body">
          <p class="delete-text">{{ t('payterm-delete-confirm') }} <strong>{{ deleteTarget?.name }}</strong>?</p>
          <div class="modal-actions">
...
const form = ref({ name: '', code: '', description: '', due_days: 30, discount_percentage: 0, discount_days: 0, is_active: true, is_default: false })
...
async function load() {
  ...
  error.value = ''
  try {
    const res = await api.get('/T0096I/')
...
function openAdd() {
  editing.value = false
  editId.value = null
  form.value = { name: '', code: '', description: '', due_days: 30, discount_percentage: 0, discount_days: 0, is_active: true, is_default: false }
...
async function saveItem() {
  ...
  try {
    ...
    if (editing.value) {
      await api.put(`/T0096I/${editId.value}`, payload)
      toast('Payment Term ' + t('saved-ok'), 'success')
    } else {
      await api.post('/T0096I/', payload)
      toast('Payment Term ' + t('saved-ok'), 'success')
    }
    closeModal()
    await load()
  } catch {
    toast(t('failed-save') + ' Payment Term', 'error')
  } finally {
    saving.value = false
  }
}
...
async function confirmDelete() {
  deleting.value = true
  try {
    await api.delete(`/T0096I/${deleteTarget.value.id}`)
    items.value = items.value.filter(i => i.id !== deleteTarget.value.id)
    toast('Payment Term deleted', 'success')
...
.checkbox-row { grid-template-columns: auto auto; }
