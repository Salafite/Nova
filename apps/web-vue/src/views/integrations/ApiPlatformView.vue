<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div><h1 class="page-title">{{ t('api-title', 'API Platform') }}</h1><p class="page-subtitle">{{ t('api-sub', 'Manage API keys, webhooks, and developer portal') }}</p></div>
      <button class="btn-primary" @click="openAdd"><span class="material-symbols-outlined">add</span> {{ t('new-api-key', 'New API Key') }}</button>
    </div>
    <SkeletonTable v-if="loading" /><ErrorState v-else-if="error" :message="error" @retry="load" />
    <div v-else-if="!items.length" class="empty-state">
      <span class="material-symbols-outlined empty-icon">api</span><p>{{ t('no-records') }}</p>
    </div>
    <div v-else class="data-card">
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>{{ t('key-name', 'Key Name') }}</th><th>{{ t('api-key', 'API Key') }}</th><th>{{ t('scopes') }}</th><th>{{ t('status') }}</th><th>{{ t('last-used', 'Last Used') }}</th><th class="text-center">{{ t('actions') }}</th></tr></thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td><strong>{{ item.name || item.key_name }}</strong></td>
              <td><span class="mono">{{ (item.api_key || item.key || '').substring(0, 16) + '...' }}</span></td>
              <td>{{ item.scopes || item.permissions || '-' }}</td>
              <td><span class="badge" :class="item.is_active ? 'badge-active' : 'badge-disabled'">{{ item.is_active ? 'Active' : 'Revoked' }}</span></td>
              ...
              <td class="text-center">
                ...
                <button class="btn-icon text-red-500" @click="revokeKey(item)"><span class="material-symbols-outlined">block</span></button>
              </td>
            </tr>
          </tbody>
        </table>
...
async function load() { loading.value = true; error.value = ''; try { const res = await api.get('/T0058I/'); items.value = res.data || [] } catch { error.value = t('failed-load') } finally { loading.value = false } }
function openAdd() { }
function editItem(item) { }
async function revokeKey(item) { try { await api.put(`/T0058I/${item.id}`, { is_active: false }); toast('Key revoked', 'success'); await load() } catch { toast(t('failed-save'), 'error') } }
onMounted(() => { load() })
</script>
<style scoped>
