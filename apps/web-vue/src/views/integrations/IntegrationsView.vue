<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div><h1 class="page-title">{{ t('integrations-title', 'Integrations') }}</h1><p class="page-subtitle">{{ t('integrations-sub', 'Manage third-party integrations and connections') }}</p></div>
      <button class="btn-primary" @click="openAdd"><span class="material-symbols-outlined">add</span> {{ t('new-integration', 'New Integration') }}</button>
    </div>
    <SkeletonTable v-if="loading" /><ErrorState v-else-if="error" :message="error" @retry="load" />
    <div v-else-if="!items.length" class="empty-state">
      <span class="material-symbols-outlined empty-icon">extension</span><p>{{ t('no-records') }}</p>
    </div>
    <div v-else class="data-card">
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>{{ t('integration-name', 'Integration') }}</th><th>{{ t('provider') }}</th><th>{{ t('type') }}</th><th>{{ t('status') }}</th><th>{{ t('last-sync', 'Last Sync') }}</th><th class="text-center">{{ t('actions') }}</th></tr></thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td><strong>{{ item.name || item.integration_name }}</strong></td>
              <td>{{ item.provider || item.service || '-' }}</td>
              <td><span class="badge badge-disabled">{{ item.type || item.integration_type || '-' }}</span></td>
              <td><span class="badge" :class="item.is_active ? 'badge-active' : 'badge-disabled'">{{ item.is_active ? 'Active' : 'Inactive' }}</span></td>
              ...
              <td class="text-center">
                ...
                <button class="btn-icon" @click="syncNow(item)" title="Sync Now"><span class="material-symbols-outlined">sync</span></button>
              </td>
            </tr>
          </tbody>
        </table>
...
async function load() { loading.value = true; error.value = ''; try { const res = await api.get('/T0056I/'); items.value = res.data || [] } catch { error.value = t('failed-load') } finally { loading.value = false } }
function openAdd() { }
function editItem(item) { }
async function syncNow(item) { try { await api.post(`/T0056I/${item.id}/sync`); toast('Sync triggered', 'success') } catch { toast(t('failed-save'), 'error') } }
onMounted(() => { load() })
</script>
<style scoped>
