<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div>
        <h2 class="page-title">{{ t('uom-title', 'Units of Measure') }}</h2>
        <p class="page-subtitle">{{ t('uom-sub', 'Manage UOM codes, names, categories, and base unit flags') }}</p>
      </div>
      <button class="btn-primary" @click="openAdd">
        <span class="material-symbols-outlined">add</span> {{ t('new-uom', 'New UOM') }}
      </button>
    </div>
    ...
    <div v-else-if="!items.length" class="empty-state">
      <span class="material-symbols-outlined empty-icon">straighten</span>
      <p>{{ t('no-records') }}</p>
    </div>
      ...
      <table class="data-table">
        <thead>
          <tr>
            ...
            <th>{{ t('name') }}</th>
            <th>{{ t('category') }}</th>
            <th class="text-center">{{ t('base-unit') }}</th>
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id">
            <td><span class="mono">{{ item.uom_code }}</span></td>
            <td class="fw-600">{{ item.uom_name }}</td>
            <td>{{ item.category }}</td>
            <td class="text-center">
              <span v-if="item.is_base_unit" class="badge badge-active">{{ t('yes') }}</span>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal">
        <div class="modal-header">
          <h3>{{ editing ? t('edit-uom', 'Edit UOM') : t('new-uom', 'New UOM') }}</h3>
          <button class="btn-icon" @click="closeModal"><span class="material-symbols-outlined">close</span></button>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label>{{ t('code') }}</label>
            <input type="text" v-model="form.uom_code" class="form-input" maxlength="10" />
          </div>
          <div class="form-group">
            <label>{{ t('name') }}</label>
            <input type="text" v-model="form.uom_name" class="form-input" maxlength="50" />
          </div>
          <div class="form-group">
          ...
          <div class="form-group checkbox-group">
            <label class="checkbox-label">
              <input type="checkbox" v-model="form.is_base_unit" />
  ...
  <ConfirmDialog v-if="confirmTarget" :title="t('confirm-delete')" :message="t('confirm-delete-msg') + ' ' + confirmTarget.uom_name" @confirm="executeDelete(confirmTarget)" @cancel="confirmTarget = null" />
  </div>
</template>
...
const form = ref({ uom_code: '', uom_name: '', category: 'Quantity', is_base_unit: false, is_active: true })
...
function openAdd() {
  editing.value = false
  editId.value = null
  form.value = { uom_code: '', uom_name: '', category: 'Quantity', is_base_unit: false, is_active: true }
...
async function saveItem() {
  ...
  try {
    if (editing.value) {
      await api.put(`/T0001I/${editId.value}`, form.value)
      toast('UOM ' + t('saved-ok'), 'success')
    } else {
      await api.post('/T0001I/', form.value)
      toast('UOM ' + t('saved-ok'), 'success')
    }
    closeModal()
    await load()
  } catch {
    toast(t('failed-save') + ' UOM', 'error')
  } finally {
    saving.value = false
  }
}
...
async function executeDelete(item) {
  ...
  try {
    await api.delete(`/T0001I/${item.id}`)
    items.value = items.value.filter(i => i.id !== item.id)
    toast('UOM ' + t('saved-ok', 'deleted'), 'success')
  } catch {
    toast(t('failed-save') + ' UOM', 'error')
  }
}
...
.checkbox-label { display: flex; align-items: center; gap: 8px; cursor: pointer; font-size: 13px; color: #333; text-transform: none; letter-spacing: 0; }
