<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div><h1 class="page-title">{{ t('documents-title', 'Documents') }}</h1><p class="page-subtitle">{{ t('documents-sub', 'Manage project documents and files') }}</p></div>
      <button class="btn-primary" @click="openAdd"><span class="material-symbols-outlined">add</span> {{ t('upload', 'Upload') }}</button>
    </div>
    <SkeletonTable v-if="loading" /><ErrorState v-else-if="error" :message="error" @retry="load" />
    <div v-else-if="!items.length" class="empty-state">
      <span class="material-symbols-outlined empty-icon">folder</span><p>{{ t('no-records') }}</p>
    </div>
    <div v-else class="data-card">
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>{{ t('name') }}</th><th>{{ t('type') }}</th><th>{{ t('size') }}</th><th>Project</th><th>{{ t('uploaded-by', 'Uploaded By') }}</th><th>{{ t('date') }}</th><th class="text-center">{{ t('actions') }}</th></tr></thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td><strong>{{ item.name || item.file_name || item.document_name }}</strong></td>
              <td><span class="badge badge-disabled">{{ item.file_type || item.document_type || '-' }}</span></td>
              <td>{{ item.file_size ? (item.file_size / 1024).toFixed(1) + ' KB' : '-' }}</td>
              <td>{{ item.project_id || item.related_id || '-' }}</td>
              <td>{{ item.uploaded_by || item.created_by || '-' }}</td>
              <td>{{ formatDate(item.created_at || item.upload_date) }}</td>
              <td class="text-center">
                <button class="btn-icon" title="Download"><span class="material-symbols-outlined">download</span></button>
...
async function load() { loading.value = true; error.value = ''; try { const res = await api.get('/T0063I/'); items.value = res.data || [] } catch { error.value = t('failed-load') } finally { loading.value = false } }
function openAdd() { }
