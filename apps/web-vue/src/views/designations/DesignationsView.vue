<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div>
        <h1 class="page-title">{{ t('desig-title') }}</h1>
        <p class="page-subtitle">{{ t('desig-sub') }}</p>
      </div>
      <button class="btn-primary" @click="openAdd">
        <span class="material-symbols-outlined">add</span> {{ t('new-desig') }}
      </button>
    </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td class="cell-mono">{{ item.designation_code }}</td>
              <td class="cell-name">{{ item.designation_name }}</td>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content">
        <div class="modal-header">
          <h3>{{ editing ? t('edit-desig') : t('new-desig') }}</h3>
          <button class="btn-icon" @click="closeModal" aria-label="Close"><span class="material-symbols-outlined">close</span></button>
        </div>
        <div class="modal-body">
          <div class="form-row">
            <div class="form-group">
              <label>{{ t('desig-code') }} <span class="required">*</span></label>
              <input type="text" v-model="form.designation_code" class="form-input" maxlength="20" />
            </div>
            <div class="form-group">
              <label>{{ t('desig-name') }} <span class="required">*</span></label>
              <input type="text" v-model="form.designation_name" class="form-input" maxlength="100" />
            </div>
          </div>
          <div class="form-group">
            <label>{{ t('desig-department') }} (ID)</label>
    ...
    <div v-if="showDelete" class="modal-overlay" @click.self="showDelete = false">
      <div class="modal-content modal-sm">
        ...
        <div class="modal-body">
          <p class="delete-text">{{ t('desig-delete-confirm') }} <strong>{{ deleteTarget?.designation_name }}</strong>?</p>
          <div class="modal-actions">
...
const form = ref({ designation_code: '', designation_name: '', department_id: null, is_active: true })
const editId = ref(null)
async function load() {
  loading.value = true; error.value = ''
  try { const res = await api.get('/T0029I/'); items.value = res.data || [] }
  catch { error.value = t('failed-load') }
  finally { loading.value = false }
}
function openAdd() {
  editing.value = false; editId.value = null
  form.value = { designation_code: '', designation_name: '', department_id: null, is_active: true }
  showModal.value = true
}
function editItem(item) {
  editing.value = true; editId.value = item.id
  form.value = { designation_code: item.designation_code, designation_name: item.designation_name, department_id: item.department_id, is_active: item.is_active }
  showModal.value = true
}
function closeModal() { showModal.value = false }
async function saveItem() {
  if (!form.value.designation_code || !form.value.designation_name) return
  saving.value = true
  try {
    const payload = { ...form.value, department_id: form.value.department_id || null }
    if (editing.value) { await api.put(`/T0029I/${editId.value}`, payload); toast(t('desig-saved'), 'success') }
    else { await api.post('/T0029I/', payload); toast(t('desig-saved'), 'success') }
...
async function confirmDelete() {
  deleting.value = true
  try { await api.delete(`/T0029I/${deleteTarget.value.id}`); items.value = items.value.filter(i => i.id !== deleteTarget.value.id); toast(t('desig-deleted'), 'success'); showDelete.value = false }
  catch { toast(t('failed-save'), 'error') }
