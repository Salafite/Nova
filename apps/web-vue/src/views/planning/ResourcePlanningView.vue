<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div><h1 class="page-title">{{ t('res-plan-title', 'Resource Planning') }}</h1><p class="page-subtitle">{{ t('res-plan-sub', 'Plan and allocate resources across projects') }}</p></div>
      <button class="btn-primary" @click="openAdd"><span class="material-symbols-outlined">add</span> {{ t('new-allocation', 'New Allocation') }}</button>
    </div>
    <SkeletonTable v-if="loading" /><ErrorState v-else-if="error" :message="error" @retry="load" />
    <div v-else-if="!items.length" class="empty-state">
      <span class="material-symbols-outlined empty-icon">groups</span><p>{{ t('no-records') }}</p>
    </div>
    <div v-else class="data-card">
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>{{ t('resource') }}</th><th>{{ t('type') }}</th><th>Project</th><th>{{ t('allocation', 'Allocation %') }}</th><th>{{ t('start-date', 'Start') }}</th><th>{{ t('end-date', 'End') }}</th><th>{{ t('status') }}</th><th class="text-center">{{ t('actions') }}</th></tr></thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td><strong>{{ item.resource_name || item.name || item.employee_name }}</strong></td>
              <td>{{ item.resource_type || item.type || '-' }}</td>
              <td>{{ item.project_name || item.project_id || '-' }}</td>
              <td class="text-center">{{ item.allocation_percentage || item.utilization || 0 }}%</td>
...
async function load() { loading.value = true; error.value = ''; try { const res = await api.get('/T0024I/'); items.value = res.data || [] } catch { error.value = t('failed-load') } finally { loading.value = false } }
function openAdd() { }
