        ...
        <table class="data-table">
          <thead>
            <tr>
              ...
              <th class="col-num">{{ t('grand-total', 'Total') }}</th>
              <th class="text-center">{{ t('status', 'Status') }}</th>
              <th>{{ t('quote-date', 'Date') }}</th>
              <th>{{ t('valid-until', 'Valid Until') }}</th>
...
async function load() {
  ...
  try {
    const res = await api.get('/T0067I/')
...
async function saveItem() {
  ...
  try {
    ...
    if (editing.value) { await api.put(`/T0067I/${editId.value}`, payload); toast(t('saved-ok', 'Saved'), 'success') }
    else { await api.post('/T0067I/', payload); toast(t('created', 'Created'), 'success') }
...
async function executeDelete() {
  ...
  try { await api.delete(`/T0067I/${item.id}`); items.value = items.value.filter(i => i.id !== item.id); toast(t('deleted', 'Deleted'), 'success') }
