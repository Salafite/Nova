        ...
        <div class="modal-body">
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <select v-model="form.customer_id" @change="onCustomerChange" required class="form-input">
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <select v-model="form.payment_term_id" class="form-input">
                <option :value="null">{{ t('no-term', 'None (Default / Inherit)') }}</option>
...
function statusBadge(status) {
  const map = { Pending: 'badge-warning', Confirmed: 'badge-info', Processing: 'badge-info', Shipped: 'badge-active', Delivered: 'badge-active', Cancelled: 'badge-inactive', Paid: 'badge-active' }
...
function getPaymentTermName(id) {
  if (!id) return '-'
  const term = paymentTerms.value.find(p => p.id === id)
  return term ? term.name : `#${id}`
}

function onCustomerChange() {
  if (!form.value.customer_id) return
  const cust = customers.value.find(c => c.id === form.value.customer_id)
  if (cust && cust.payment_term_id && !editing.value && !form.value.payment_term_id) {
    form.value.payment_term_id = cust.payment_term_id
  }
}
