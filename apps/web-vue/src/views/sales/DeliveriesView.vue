<template>
  <div :dir="dir">
    <div class="page-header">
      <div>
        <h2 class="page-title">{{ t('deliveries-title', 'Deliveries') }}</h2>
        <p class="page-subtitle">{{ t('deliveries-sub', 'Manage order shipments and deliveries') }}</p>
      </div>
      <button class="btn-primary" @click="openAdd">
        <span class="material-symbols-outlined">add</span> {{ t('new-delivery', 'New Delivery') }}
      </button>
    </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              ...
              <td>{{ orderLink(item.sales_order_id) }}</td>
        ...
        <div class="modal-body">
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <select v-model="form.sales_order_id" required class="form-input">
    ...
    <ConfirmDialog v-if="confirmTarget" :title="t('confirm-delete', 'Delete?')" :message="t('confirm-delete-msg', 'Delete delivery') + ' ' + confirmTarget.delivery_number + '?'" @confirm="executeDelete" @cancel="confirmTarget = null" />
  </div>
</template>
...
async function load() {
  ...
  try {
    const [delRes, ordRes, whRes] = await Promise.all([
      api.get('/T0077I/'),
      api.get('/T0012I/'),
      api.get('/T0008I/')
    ])
...
function openAdd() {
  ...
  form.value = { delivery_number: '', sales_order_id: null, delivery_date: today(), warehouse_id: null, status: 'Draft', notes: '' }
  showModal.value = true
}

function editItem(item) {
  editing.value = true; editId.value = item.id
  form.value = { delivery_number: item.delivery_number, sales_order_id: item.sales_order_id, delivery_date: item.delivery_date || today(), warehouse_id: item.warehouse_id, status: item.status || 'Draft', notes: item.notes || '' }
  showModal.value = true
}
...
async function saveItem() {
  saving.value = true
  try {
    const payload = { ...form.value, notes: form.value.notes || null, warehouse_id: form.value.warehouse_id || null }
    if (editing.value) { await api.put(`/T0077I/${editId.value}`, payload); toast(t('saved-ok', 'Saved'), 'success') }
    else { await api.post('/T0077I/', payload); toast(t('created', 'Created'), 'success') }
...
async function executeDelete() {
  ...
  try { await api.delete(`/T0077I/${item.id}`); items.value = items.value.filter(i => i.id !== item.id); toast(t('deleted', 'Deleted'), 'success') }
