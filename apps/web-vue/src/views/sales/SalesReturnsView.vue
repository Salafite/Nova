  ...
  <div :dir="dir">
    <div class="page-header">
      ...
      <button class="btn-primary" @click="openAdd">
        <span class="material-symbols-outlined">add</span> {{ t('new-return', 'New Return') }}
      </button>
    </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td class="cell-order">{{ item.return_number }}</td>
              <td>{{ customerName(item.customer_id) }}</td>
              <td class="cell-mono">{{ orderNumber(item.sales_order_id) }}</td>
        ...
        <div class="modal-body">
          <div class="form-row">
            <div class="form-group">
              <label>{{ t('return-number', 'Return #') }} <span class="required">*</span></label>
              <input type="text" v-model="form.return_number" required class="form-input" maxlength="30" />
            </div>
            <div class="form-group">
          ...
          <div class="form-row">
            <div class="form-group">
              <label>{{ t('sales-order', 'Sales Order') }}</label>
              <select v-model="form.sales_order_id" class="form-input">
                <option value="">-- None --</option>
                <option v-for="o in orders" :key="o.id" :value="o.id">{{ o.order_number }}</option>
              </select>
            </div>
            <div class="form-group">
              <label>{{ t('return-date', 'Return Date') }}</label>
              <input type="date" v-model="form.return_date" class="form-input" />
            </div>
          </div>
          <div class="form-group">
            <label>{{ t('reason', 'Reason') }}</label>
            <textarea v-model="form.reason" class="form-input" rows="2"></textarea>
          </div>
          <div class="form-group">
    ...
    <ConfirmDialog v-if="confirmTarget" :title="t('confirm-delete', 'Delete?')" :message="t('confirm-delete-msg', 'Delete return') + ' ' + confirmTarget.return_number + '?'" @confirm="executeDelete" @cancel="confirmTarget = null" />
  </div>
</template>
...
const form = ref({ return_number: '', customer_id: null, sales_order_id: null, return_date: '', status: 'Draft', reason: '', notes: '' })
const editId = ref(null)
const confirmTarget = ref(null)

function statusBadge(status) {
  const map = { Draft: 'badge-inactive', Approved: 'badge-info', Received: 'badge-active', Cancelled: 'badge-inactive' }
...
function openAdd() {
  ...
  form.value = { return_number: '', customer_id: null, sales_order_id: null, return_date: today(), status: 'Draft', reason: '', notes: '' }
  showModal.value = true
}

function editItem(item) {
  editing.value = true; editId.value = item.id
  form.value = { return_number: item.return_number, customer_id: item.customer_id, sales_order_id: item.sales_order_id, return_date: item.return_date || today(), status: item.status || 'Draft', reason: item.reason || '', notes: item.notes || '' }
  showModal.value = true
}
...
async function saveItem() {
  saving.value = true
  try {
    const payload = { ...form.value, notes: form.value.notes || null, reason: form.value.reason || null, sales_order_id: form.value.sales_order_id || null }
    if (editing.value) { await api.put(`/T0079I/${editId.value}`, payload); toast(t('saved-ok', 'Saved'), 'success') }
    else { await api.post('/T0079I/', payload); toast(t('created', 'Created'), 'success') }
...
async function approveReturn(item) {
  try { await api.post(`/T0079I/${item.id}/approve`); toast(t('approved', 'Return approved'), 'success'); await load() }
  catch { toast(t('failed-action', 'Failed to approve'), 'error') }
}

async function receiveReturn(item) {
  try { await api.post(`/T0079I/${item.id}/receive`); toast(t('received', 'Return received'), 'success'); await load() }
  catch { toast(t('failed-action', 'Failed to receive'), 'error') }
}

function deleteItem(item) { confirmTarget.value = item }
async function executeDelete() {
  const item = confirmTarget.value; confirmTarget.value = null
  try { await api.delete(`/T0079I/${item.id}`); items.value = items.value.filter(i => i.id !== item.id); toast(t('deleted', 'Deleted'), 'success') }
