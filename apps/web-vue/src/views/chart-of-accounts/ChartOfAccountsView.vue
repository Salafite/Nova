<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div>
        <h1 class="page-title">{{ t('coa-title') }}</h1>
        <p class="page-subtitle">{{ t('coa-sub') }}</p>
      </div>
      <button class="btn-primary" @click="openAdd">
        <span class="material-symbols-outlined">add</span> {{ t('new-account') }}
      </button>
    </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td class="cell-mono">{{ item.account_code }}</td>
              <td class="cell-name">{{ item.account_name }}</td>
              <td><span :class="'type-badge type-' + typeClass(item.account_type)">{{ item.account_type }}</span></td>
              <td>{{ item.currency }}</td>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content">
        <div class="modal-header">
          <h3>{{ editing ? t('edit-account') : t('new-account') }}</h3>
          <button class="btn-icon" @click="closeModal" aria-label="Close"><span class="material-symbols-outlined">close</span></button>
        </div>
        <div class="modal-body">
          <div class="form-row">
            <div class="form-group">
              <label>{{ t('coa-code') }} <span class="required">*</span></label>
              <input type="text" v-model="form.account_code" class="form-input" maxlength="20" />
            </div>
            <div class="form-group">
              <label>{{ t('coa-name') }} <span class="required">*</span></label>
              <input type="text" v-model="form.account_name" class="form-input" maxlength="100" />
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label>{{ t('coa-type') }} <span class="required">*</span></label>
    ...
    <div v-if="showDelete" class="modal-overlay" @click.self="showDelete = false">
      <div class="modal-content modal-sm">
        ...
        <div class="modal-body">
          <p class="delete-text">{{ t('coa-delete-confirm') }} <strong>{{ deleteTarget?.account_name }}</strong>?</p>
          <div class="modal-actions">
...
const form = ref({ account_code: '', account_name: '', account_type: 'Asset', currency: 'USD', is_active: true })
const editId = ref(null)

function typeClass(type) {
  const map = { Asset: 'asset', Liability: 'liability', Equity: 'equity', Revenue: 'revenue', Expense: 'expense' }
...
async function load() {
  ...
  error.value = ''
  try {
    const res = await api.get('/T0026I/')
...
function openAdd() {
  editing.value = false
  editId.value = null
  form.value = { account_code: '', account_name: '', account_type: 'Asset', currency: 'USD', is_active: true }
...
async function saveItem() {
  if (!form.value.account_code || !form.value.account_name) return
  saving.value = true
  try {
    if (editing.value) {
      await api.put(`/T0026I/${editId.value}`, form.value)
      toast('Account ' + t('saved-ok'), 'success')
    } else {
      await api.post('/T0026I/', form.value)
      toast('Account ' + t('saved-ok'), 'success')
    }
    closeModal()
    await load()
  } catch {
    toast(t('failed-save') + ' Account', 'error')
  } finally {
    saving.value = false
  }
}
...
async function confirmDelete() {
  deleting.value = true
  try {
    await api.delete(`/T0026I/${deleteTarget.value.id}`)
...
.type-asset { background: #dbeafe; color: #2563eb; }
.type-liability { background: #fef3c7; color: #d97706; }
.type-equity { background: #ede9fe; color: #7c3aed; }
.type-revenue { background: #dcfce7; color: #16a34a; }
