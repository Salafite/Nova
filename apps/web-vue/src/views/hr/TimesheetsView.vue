<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div><h1 class="page-title">{{ t('timesheets-title', 'Timesheets') }}</h1><p class="page-subtitle">{{ t('timesheets-sub', 'Track employee work hours and timesheets') }}</p></div>
      <button class="btn-primary" @click="openAdd"><span class="material-symbols-outlined">add</span> {{ t('new-entry', 'New Entry') }}</button>
    </div>
    <SkeletonTable v-if="loading" /><ErrorState v-else-if="error" :message="error" @retry="load" />
    <div v-else-if="!items.length" class="empty-state">
      <span class="material-symbols-outlined empty-icon">timer</span><p>{{ t('no-records') }}</p>
    </div>
    <div v-else class="data-card">
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>{{ t('employee') }}</th><th>{{ t('date') }}</th><th>{{ t('clock-in', 'Clock In') }}</th><th>{{ t('clock-out', 'Clock Out') }}</th><th class="text-center">{{ t('hours') }}</th><th>{{ t('project') }}</th><th>{{ t('status') }}</th><th class="text-center">{{ t('actions') }}</th></tr></thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td>{{ item.employee_name || item.employee_id || '-' }}</td>
              <td>{{ formatDate(item.date || item.entry_date) }}</td>
              <td>{{ item.clock_in || item.start_time || '-' }}</td>
              <td>{{ item.clock_out || item.end_time || '-' }}</td>
              <td class="text-center">{{ item.total_hours || item.hours || '-' }}</td>
...
async function load() { loading.value = true; error.value = ''; try { const res = await api.get('/T0030I/'); items.value = res.data || [] } catch { error.value = t('failed-load') } finally { loading.value = false } }
function openAdd() { }
