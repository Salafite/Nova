<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div><h1 class="page-title">{{ t('shopfloor-title', 'Shop Floor') }}</h1><p class="page-subtitle">{{ t('shopfloor-sub', 'Real-time shop floor operations and monitoring') }}</p></div>
    </div>
    <SkeletonTable v-if="loading" /><ErrorState v-else-if="error" :message="error" @retry="load" />
    <div v-else-if="!items.length" class="empty-state">
      <span class="material-symbols-outlined empty-icon">construction</span><p>{{ t('no-records') }}</p>
    </div>
    <div v-else class="data-card">
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>{{ t('job-number', 'Job #') }}</th><th>{{ t('product') }}</th><th>{{ t('workstation') }}</th><th>{{ t('operator') }}</th><th>{{ t('status') }}</th><th class="text-center">{{ t('qty', 'Qty') }}</th><th class="text-center">{{ t('completed', 'Completed') }}</th><th class="text-center">{{ t('actions') }}</th></tr></thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td><span class="mono">{{ item.job_number || item.code || item.id }}</span></td>
              <td>{{ item.product_name || item.product_id || '-' }}</td>
              <td>{{ item.workstation || item.work_center || '-' }}</td>
              <td>{{ item.operator || item.assigned_to || '-' }}</td>
              <td><span class="badge" :class="'badge-' + (item.status || '').toLowerCase()">{{ item.status || 'Planned' }}</span></td>
              <td class="text-center">{{ item.quantity || item.total_qty || 0 }}</td>
              <td class="text-center">{{ item.completed_qty || 0 }}</td>
...
async function load() { loading.value = true; error.value = ''; try { const res = await api.get('/T0020I/'); items.value = res.data || [] } catch { error.value = t('failed-load') } finally { loading.value = false } }
