<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div>
        <h1 class="page-title">{{ t('serial-title', 'Serial Numbers') }}</h1>
        <p class="page-subtitle">{{ t('serial-sub', 'Manage serialized inventory tracking') }}</p>
      </div>
      <button class="btn-primary" @click="openAdd">
        <span class="material-symbols-outlined">add</span> {{ t('new-serial', 'New Serial') }}
      </button>
    </div>
    <SkeletonTable v-if="loading" />
    <ErrorState v-else-if="error" :message="error" @retry="load" />
    <div v-else-if="!items.length" class="empty-state">
      <span class="material-symbols-outlined empty-icon">qr_code</span>
      <p>{{ t('no-records') }}</p>
    </div>
    <div v-else class="data-card">
      <div class="table-wrap">
        <table class="data-table">
          <thead>
            <tr>
              <th>{{ t('serial-number', 'Serial #') }}</th>
              <th>Product</th>
              <th>Warehouse</th>
              <th>{{ t('status') }}</th>
              <th>{{ t('batch-number', 'Batch') }}</th>
              <th class="text-center">{{ t('actions') }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td><span class="mono">{{ item.serial_number }}</span></td>
              <td>{{ productName(item.product_id) }}</td>
              <td>{{ warehouseName(item.warehouse_id) }}</td>
              <td><span class="badge" :class="item.status === 'available' ? 'badge-active' : 'badge-disabled'">{{ item.status }}</span></td>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal">
        <div class="modal-header">
          <h3>{{ editing ? t('edit-serial', 'Edit Serial') : t('new-serial', 'New Serial') }}</h3>
          <button class="btn-icon" @click="closeModal"><span class="material-symbols-outlined">close</span></button>
        </div>
        <div class="modal-body">
          <div class="form-group"><label>{{ t('serial-number', 'Serial #') }} <span class="text-red-500">*</span></label><input type="text" v-model="form.serial_number" class="form-input" /></div>
          ...
          <div class="form-group"><label>{{ t('status') }}</label><select v-model="form.status" class="form-input"><option value="available">Available</option><option value="sold">Sold</option><option value="returned">Returned</option><option value="scrap">Scrap</option></select></div>
        </div>
    ...
    <ConfirmDialog v-if="confirmTarget" :title="t('confirm-delete')" :message="t('confirm-delete-msg') + ' ' + confirmTarget.serial_number" @confirm="executeDelete(confirmTarget)" @cancel="confirmTarget = null" />
  </div>
</template>
<script setup>
...
const form = ref({ serial_number: '', product_id: null, warehouse_id: null, status: 'available' })
...
async function load() { loading.value = true; error.value = ''; try { const res = await api.get('/T0106I/'); items.value = res.data || [] } catch { error.value = t('failed-load') } finally { loading.value = false } }
function openAdd() { editing.value = false; editId.value = null; form.value = { serial_number: '', product_id: null, warehouse_id: null, status: 'available' }; showModal.value = true }
function editItem(item) { editing.value = true; editId.value = item.id; form.value = { serial_number: item.serial_number, product_id: item.product_id, warehouse_id: item.warehouse_id, status: item.status || 'available' }; showModal.value = true }
function closeModal() { showModal.value = false }
async function saveItem() { saving.value = true; try { if (editing.value) { await api.put(`/T0106I/${editId.value}`, form.value); toast('Serial ' + t('saved-ok'), 'success') } else { await api.post('/T0106I/', form.value); toast('Serial ' + t('saved-ok'), 'success') } closeModal(); await load() } catch { toast(t('failed-save'), 'error') } finally { saving.value = false } }
async function deleteItem(item) { confirmTarget.value = item }
async function executeDelete(item) { confirmTarget.value = null; try { await api.delete(`/T0106I/${item.id}`); items.value = items.value.filter(i => i.id !== item.id); toast('Serial deleted', 'success') } catch { toast(t('failed-save'), 'error') } }
