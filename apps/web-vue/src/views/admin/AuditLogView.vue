<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div><h1 class="page-title">{{ t('audit-title', 'Audit Log') }}</h1><p class="page-subtitle">{{ t('audit-sub', 'View system audit trail and user activity') }}</p></div>
    </div>
    <SkeletonTable v-if="loading" /><ErrorState v-else-if="error" :message="error" @retry="load" />
    <div v-else-if="!items.length" class="empty-state">
      <span class="material-symbols-outlined empty-icon">history</span><p>{{ t('no-records') }}</p>
    </div>
    <div v-else class="data-card">
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>{{ t('timestamp') }}</th><th>{{ t('user') }}</th><th>{{ t('action') }}</th><th>{{ t('resource') }}</th><th>{{ t('details') }}</th></tr></thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td class="mono">{{ formatDate(item.created_at || item.timestamp) }}</td>
              <td>{{ item.username || item.user_id || '-' }}</td>
              <td><span class="badge badge-disabled">{{ item.action || item.event_type }}</span></td>
              <td>{{ item.resource || item.table_name || '-' }}</td>
              <td class="cell-desc">{{ item.description || item.details || '-' }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
async function load() { loading.value = true; error.value = ''; try { const res = await api.get('/T0022I/'); items.value = res.data || [] } catch { error.value = t('failed-load') } finally { loading.value = false } }
onMounted(() => { load() })
</script>
<style scoped>
