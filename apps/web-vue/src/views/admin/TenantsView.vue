<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div><h1 class="page-title">{{ t('tenants-title', 'Multi-Tenant Management') }}</h1><p class="page-subtitle">{{ t('tenants-sub', 'Manage tenants/businesses in the system') }}</p></div>
    </div>
    <SkeletonTable v-if="loading" /><ErrorState v-else-if="error" :message="error" @retry="load" />
    <div v-else-if="!items.length" class="empty-state">
      <span class="material-symbols-outlined empty-icon">business</span><p>{{ t('no-records') }}</p>
    </div>
    <div v-else class="data-card">
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>{{ t('business-name', 'Business') }}</th><th>{{ t('code') }}</th><th>{{ t('email') }}</th><th>{{ t('plan') }}</th><th>{{ t('status') }}</th><th class="text-center">{{ t('actions') }}</th></tr></thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td><strong>{{ item.name || item.business_name || item.company_name }}</strong></td>
              <td><span class="mono">{{ item.code || item.business_code || '-' }}</span></td>
              <td>{{ item.email || '-' }}</td>
              <td>{{ item.plan || item.subscription_plan || '-' }}</td>
...
async function load() { loading.value = true; error.value = ''; try { const res = await api.get('/T0059I/'); items.value = res.data || [] } catch { error.value = t('failed-load') } finally { loading.value = false } }
