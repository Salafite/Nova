<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div><h1 class="page-title">{{ t('workflow-title', 'Workflow Automation') }}</h1><p class="page-subtitle">{{ t('workflow-sub', 'Design and manage automated workflows') }}</p></div>
    </div>
    <SkeletonTable v-if="loading" /><ErrorState v-else-if="error" :message="error" @retry="load" />
    <div v-else-if="!items.length" class="empty-state">
      <span class="material-symbols-outlined empty-icon">alt_route</span><p>{{ t('no-records') }}</p>
    </div>
    <div v-else class="data-card">
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>{{ t('workflow-name', 'Workflow') }}</th><th>{{ t('trigger') }}</th><th>{{ t('module') }}</th><th>{{ t('status') }}</th><th class="text-center">{{ t('actions') }}</th></tr></thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td><strong>{{ item.name || item.workflow_name }}</strong></td>
              <td>{{ item.trigger_event || item.trigger || '-' }}</td>
              <td>{{ item.module || '-' }}</td>
...
async function load() { loading.value = true; error.value = ''; try { const res = await api.get('/T0061I/'); items.value = res.data || [] } catch { error.value = t('failed-load') } finally { loading.value = false } }
