  ...
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      ...
      <button class="btn-primary" @click="openAdd">
        <span class="material-symbols-outlined">add</span> {{ t('new-customer') }}
      </button>
    </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td class="cell-name"><a class="name-link" @click="router.push(`/customers/${item.id}`)">{{ item.name }}</a></td>
              <td>{{ item.group_name }}</td>
              <td class="cell-mono">{{ item.phone || '-' }}</td>
              <td class="cell-mono">{{ item.email || '-' }}</td>
              <td class="col-num">${{ (item.credit_limit || 0).toFixed(2) }}</td>
              <td class="col-num">${{ (item.balance || 0).toFixed(2) }}</td>
...
async function saveItem() {
  ...
  try {
    ...
    if (editing.value) {
      ...
      toast('Customer ' + t('saved-ok'), 'success')
    } else {
    ...
    toast(t('failed-save') + ' Customer', 'error')
  } finally {
    saving.value = false
  }
}
...
.name-link { color: #5d3fd3; cursor: pointer; }
