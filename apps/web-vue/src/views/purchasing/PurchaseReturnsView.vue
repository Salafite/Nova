  ...
  <div :dir="dir">
    <div class="page-header">
      ...
      <button class="btn-primary" @click="openAdd">
        <span class="material-symbols-outlined">add</span> {{ t('new-preturn', 'New Return') }}
      </button>
    </div>
        ...
        <table class="data-table">
          <thead>
            <tr>
              ...
              <th>{{ t('purchase-order', 'Purchase Order') }}</th>
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              ...
              <td class="cell-mono">{{ poNumber(item.purchase_order_id) }}</td>
...
const form = ref({ return_number: '', supplier_id: null, purchase_order_id: null, return_date: '', status: 'Draft', reason: '', notes: '' })
...
function openAdd() {
  ...
  form.value = { return_number: '', supplier_id: null, purchase_order_id: null, return_date: today(), status: 'Draft', reason: '', notes: '' }
  showModal.value = true
}

function editItem(item) {
  editing.value = true; editId.value = item.id
  form.value = { return_number: item.return_number, supplier_id: item.supplier_id, purchase_order_id: item.purchase_order_id, return_date: item.return_date || today(), status: item.status || 'Draft', reason: item.reason || '', notes: item.notes || '' }
  showModal.value = true
}
...
async function saveItem() {
  saving.value = true
  try {
    const payload = { ...form.value, notes: form.value.notes || null, reason: form.value.reason || null, purchase_order_id: form.value.purchase_order_id || null }
    if (editing.value) { await api.put(`/T0081I/${editId.value}`, payload); toast(t('saved-ok', 'Saved'), 'success') }
    else { await api.post('/T0081I/', payload); toast(t('created', 'Created'), 'success') }
...
async function approveReturn(item) {
  try { await api.post(`/T0081I/${item.id}/approve`); toast(t('approved', 'Return approved'), 'success'); await load() }
  catch { toast(t('failed-action', 'Failed to approve'), 'error') }
}

async function receiveReturn(item) {
  try { await api.post(`/T0081I/${item.id}/receive`); toast(t('received', 'Return received'), 'success'); await load() }
...
async function executeDelete() {
  ...
  try { await api.delete(`/T0081I/${item.id}`); items.value = items.value.filter(i => i.id !== item.id); toast(t('deleted', 'Deleted'), 'success') }
