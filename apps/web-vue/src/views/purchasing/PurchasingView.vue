<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div>
        <h2 class="page-title">{{ t('purchasing-title') }}</h2>
        <p class="page-subtitle">{{ t('purchasing-sub') }}</p>
      </div>
      <button class="btn-primary" @click="openAdd">
        <span class="material-symbols-outlined">add</span> {{ t('new-purchase-order') }}
      </button>
    </div>
    ...
    <div v-else-if="!items.length" class="empty-state">
      ...
      <p>{{ t('no-records') }}</p>
      <button class="btn-primary" @click="openAdd">{{ t('new-purchase-order') }}</button>
    </div>
    ...
    <div v-else class="data-card">
      <div class="table-wrap">
        <table class="data-table">
          <thead>
            <tr>
              <th>{{ t('purchase-order-number') }}</th>
              <th>{{ t('purchase-supplier') }}</th>
              <th class="col-num">{{ t('purchase-total') }}</th>
              <th class="text-center">{{ t('status') }}</th>
              <th>{{ t('purchase-order-date') }}</th>
              <th>{{ t('purchase-expected-date') }}</th>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="form-row">
            <div class="form-group">
              <label>{{ t('purchase-order-date') }}</label>
              <input type="date" v-model="form.order_date" class="form-input" />
            </div>
            <div class="form-group">
              <label>{{ t('purchase-expected-date') }}</label>
...
async function load() {
  ...
  error.value = ''
  try {
    const res = await api.get('/T0014I/')
...
async function saveItem() {
  ...
  try {
    ...
    if (editing.value) {
      ...
      toast('Purchase Order ' + t('saved-ok'), 'success')
    } else {
    ...
    toast(t('failed-save') + ' Purchase Order', 'error')
  } finally {
    saving.value = false
  }
}
...
.cell-order { font-family: monospace; font-weight: 600; color: #5d3fd3; }
