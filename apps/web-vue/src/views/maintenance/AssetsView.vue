<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div><h1 class="page-title">{{ t('assets-title', 'Assets') }}</h1><p class="page-subtitle">{{ t('assets-sub', 'Manage maintenance assets and equipment') }}</p></div>
      <button class="btn-primary" @click="openAdd"><span class="material-symbols-outlined">add</span> {{ t('new-asset', 'New Asset') }}</button>
    </div>
    <SkeletonTable v-if="loading" /><ErrorState v-else-if="error" :message="error" @retry="load" />
    <div v-else-if="!items.length" class="empty-state">
      <span class="material-symbols-outlined empty-icon">build</span><p>{{ t('no-records') }}</p>
    </div>
    <div v-else class="data-card">
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>{{ t('asset-name', 'Asset') }}</th><th>{{ t('asset-code', 'Code') }}</th><th>{{ t('asset-type', 'Type') }}</th><th>{{ t('location') }}</th><th>{{ t('status') }}</th><th class="text-center">{{ t('actions') }}</th></tr></thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td><strong>{{ item.name || item.asset_name }}</strong></td>
              <td><span class="mono">{{ item.code || item.asset_code }}</span></td>
...
async function load() { loading.value = true; error.value = ''; try { const res = await api.get('/T0041I/'); items.value = res.data || [] } catch { error.value = t('failed-load') } finally { loading.value = false } }
function openAdd() { }
...
.badge-under_maintenance { background: #fff3e0; color: #e65100; }
