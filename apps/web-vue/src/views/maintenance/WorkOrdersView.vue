<template>
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div><h1 class="page-title">{{ t('wo-title', 'Work Orders') }}</h1><p class="page-subtitle">{{ t('wo-sub', 'Manage maintenance work orders') }}</p></div>
      <button class="btn-primary" @click="openAdd"><span class="material-symbols-outlined">add</span> {{ t('new-wo', 'New Work Order') }}</button>
    </div>
    <SkeletonTable v-if="loading" /><ErrorState v-else-if="error" :message="error" @retry="load" />
    <div v-else-if="!items.length" class="empty-state">
      <span class="material-symbols-outlined empty-icon">assignment</span><p>{{ t('no-records') }}</p>
    </div>
    <div v-else class="data-card">
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>{{ t('wo-number', 'WO #') }}</th><th>{{ t('title') }}</th><th>{{ t('asset') }}</th><th>{{ t('priority') }}</th><th>{{ t('status') }}</th><th>{{ t('assigned-to') }}</th><th>{{ t('due-date', 'Due') }}</th><th class="text-center">{{ t('actions') }}</th></tr></thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td><span class="mono">{{ item.work_order_number || item.code || item.id }}</span></td>
              <td><strong>{{ item.title || item.name || item.description }}</strong></td>
              <td>{{ item.asset_id || '-' }}</td>
...
async function load() { loading.value = true; error.value = ''; try { const res = await api.get('/T0043I/'); items.value = res.data || [] } catch { error.value = t('failed-load') } finally { loading.value = false } }
function openAdd() { }
