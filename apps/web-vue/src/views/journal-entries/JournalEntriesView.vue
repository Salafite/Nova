  ...
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      ...
      <button class="btn-primary" @click="openAdd">
        <span class="material-symbols-outlined">add</span> {{ t('new-je') }}
      </button>
    </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              ...
              <td class="cell-mono">{{ item.entry_date }}</td>
        ...
        <div class="modal-body">
          <div class="form-row">
            <div class="form-group">
              <label>{{ t('je-date') }} <span class="required">*</span></label>
              <input type="date" v-model="form.entry_date" class="form-input" />
            </div>
            <div class="form-group">
          ...
          <div class="form-group">
            <label>{{ t('description') }} <span class="required">*</span></label>
            <textarea v-model="form.description" class="form-input" maxlength="255" rows="3"></textarea>
          </div>
          <div class="form-row">
            <div class="form-group">
...
const form = ref({ entry_date: '', reference: '', description: '', status: 'Draft' })
const editId = ref(null)
function statusBadge(status) {
  if (status === 'Posted') return 'badge badge-active'
...
async function load() {
  ...
  try { const res = await api.get('/T0027I/'); items.value = res.data || [] }
...
function openAdd() {
  ...
  form.value = { entry_date: today, reference: '', description: '', status: 'Draft' }
  showModal.value = true
}
function editItem(item) {
  editing.value = true; editId.value = item.id
  form.value = { entry_date: item.entry_date, reference: item.reference, description: item.description, status: item.status }
  showModal.value = true
}
function closeModal() { showModal.value = false }
async function saveItem() {
  if (!form.value.entry_date || !form.value.description) return
  saving.value = true
  try {
    const payload = { ...form.value, reference: form.value.reference || null }
    if (editing.value) { await api.put(`/T0027I/${editId.value}`, payload); toast('Journal Entry ' + t('saved-ok'), 'success') }
    else { await api.post('/T0027I/', payload); toast('Journal Entry ' + t('saved-ok'), 'success') }
    closeModal(); await load()
  } catch { toast(t('failed-save') + ' Journal Entry', 'error') }
  finally { saving.value = false }
}
function deleteItem(item) { deleteTarget.value = item; showDelete.value = true }
async function confirmDelete() {
  deleting.value = true
  try { await api.delete(`/T0027I/${deleteTarget.value.id}`); items.value = items.value.filter(i => i.id !== deleteTarget.value.id); toast('Journal Entry deleted', 'success'); showDelete.value = false }
  catch { toast(t('failed-save') + ' Journal Entry', 'error') }
