  ...
  <div :dir="dir">
    <div class="flex justify-between items-center mb-6">
      <div>
        ...
        <p class="page-subtitle">{{ t('dept-sub') }}</p>
      </div>
      <button class="btn-primary" @click="openAdd">
        <span class="material-symbols-outlined">add</span> {{ t('new-dept') }}
      </button>
    </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td class="cell-mono">{{ item.department_code }}</td>
              <td class="cell-name">{{ item.department_name }}</td>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content">
        <div class="modal-header">
          <h3>{{ editing ? t('edit-dept') : t('new-dept') }}</h3>
          <button class="btn-icon" @click="closeModal" aria-label="Close"><span class="material-symbols-outlined">close</span></button>
        </div>
        <div class="modal-body">
          <div class="form-row">
            <div class="form-group">
              <label>{{ t('dept-code') }} <span class="required">*</span></label>
              <input type="text" v-model="form.department_code" class="form-input" maxlength="20" />
            </div>
            <div class="form-group">
              <label>{{ t('dept-name') }} <span class="required">*</span></label>
              <input type="text" v-model="form.department_name" class="form-input" maxlength="100" />
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label>{{ t('dept-parent') }} (ID)</label>
              <input type="number" min="0" v-model.number="form.parent_id" class="form-input" />
            </div>
            <div class="form-group">
              <label>{{ t('dept-manager') }} (ID)</label>
    ...
    <div v-if="showDelete" class="modal-overlay" @click.self="showDelete = false">
      <div class="modal-content modal-sm">
        ...
        <div class="modal-body">
          <p class="delete-text">{{ t('dept-delete-confirm') }} <strong>{{ deleteTarget?.department_name }}</strong>?</p>
          <div class="modal-actions">
...
const form = ref({ department_code: '', department_name: '', parent_id: null, manager_id: null, is_active: true })
const editId = ref(null)
async function load() {
  loading.value = true; error.value = ''
  try { const res = await api.get('/T0028I/'); items.value = res.data || [] }
  catch { error.value = t('failed-load') }
  finally { loading.value = false }
}
function openAdd() {
  editing.value = false; editId.value = null
  form.value = { department_code: '', department_name: '', parent_id: null, manager_id: null, is_active: true }
  showModal.value = true
}
function editItem(item) {
  editing.value = true; editId.value = item.id
  form.value = { department_code: item.department_code, department_name: item.department_name, parent_id: item.parent_id, manager_id: item.manager_id, is_active: item.is_active }
  showModal.value = true
}
function closeModal() { showModal.value = false }
async function saveItem() {
  if (!form.value.department_code || !form.value.department_name) return
  saving.value = true
  try {
    const payload = { ...form.value, parent_id: form.value.parent_id || null, manager_id: form.value.manager_id || null }
    if (editing.value) { await api.put(`/T0028I/${editId.value}`, payload); toast(t('dept-saved'), 'success') }
    else { await api.post('/T0028I/', payload); toast(t('dept-saved'), 'success') }
...
async function confirmDelete() {
  deleting.value = true
  try { await api.delete(`/T0028I/${deleteTarget.value.id}`); items.value = items.value.filter(i => i.id !== deleteTarget.value.id); toast(t('dept-deleted'), 'success'); showDelete.value = false }
  catch { toast(t('failed-save'), 'error') }
