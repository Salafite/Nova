...
class ParsedStatement:
    ...
    def __getitem__(self, key: str) -> Any:
        if key == "transactions":
            return [t.to_dict() if hasattr(t, "to_dict") else t for t in self.transactions]
        return getattr(self, key)
...
class BankStatementParser:
    ...
    def parse_file(
        ...
        """Classmethod helper to parse file content and return statement dict structure."""
        parser = cls()
        stmt = parser.parse(file_content, file_name=file_name, file_type=file_type)
        return stmt.to_dict()
