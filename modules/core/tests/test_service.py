...
class TestAuthenticateUser:
    def test_returns_user_on_valid_credentials(self, mock_bcrypt, sample_user):
        with patch('auth.service.get_user_by_username', return_value=sample_user):
            with patch('auth.service.update_last_login'):
    ...
    def test_returns_none_on_unknown_user(self, mock_db):
        with patch('auth.service.get_user_by_username', return_value=None):
...
class TestRefresh:
    def test_returns_new_tokens_on_valid_refresh(self, valid_refresh_token):
        with patch('auth.service.get_user_by_id') as mock_get:
    ...
    def test_returns_none_when_user_not_found(self, valid_refresh_token):
        with patch('auth.service.get_user_by_id', return_value=None):
            result = refresh(valid_refresh_token)
            assert result is None
